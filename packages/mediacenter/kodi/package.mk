...
if [ "$RETROPLAYER_SUPPORT" = yes ]; then
  PKG_DEPENDS_TARGET="$PKG_DEPENDS_TARGET kodi-game-controllers"

  pre_patch() {
    PATCHES="kodi_garbear-retroplayer-15.2.patch"
    [ ! "$OPENGL" = "no" ] && PATCHES="$PATCHES kodi_garbear-retroplayer-gl.patch"
    echo $PATCHES
    for i in $PATCHES; do
