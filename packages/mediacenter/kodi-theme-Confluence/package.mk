...
if [ "$RETROPLAYER_SUPPORT" = yes ]; then
  pre_patch() {
    ...
    cat $PKG_DIR/patches/kodi-theme-Confluence_garbear-retroplayer-15.2.patch | git apply --directory=`echo "$PKG_BUILD" | cut -f1 -d\ ` -p3 --verbose --whitespace=nowarn >&$VERBOSE_OUT
  }
fi
